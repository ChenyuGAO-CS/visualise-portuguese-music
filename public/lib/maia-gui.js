var mg = (function () {
  ...
  class Button {
    ...
    draw_piano_roll_with_play_head(prm, note_playing){
      const self = this

      const hier_bars = prm.hier.bars
      const compObjNotes = prm.compObj.notes
      const maxOntimeCurrent = compObjNotes[compObjNotes.length-1].ontime
      let totalBarCount = Math.ceil((compObjNotes[compObjNotes.length-1].offtime)/4)

      const interval_two_octave = self.sk.map(
        36, prm.screenBRepresents, prm.screenTRepresents,
        prm.canvasHeight/2 - 2*prm.gridMargin, prm.gridMargin
      ) - 
      self.sk.map(
        48, prm.screenBRepresents, prm.screenTRepresents,
        prm.canvasHeight/2 - 2*prm.gridMargin, prm.gridMargin
      )
      ...
      // Draw occurrences 
      const occ = prm.hier
      const hier_gt = occ["occurrences"] // Read the first occ set. 
      const occ_key = Object.keys(hier_gt)
      occ_key.forEach(function(k){
        const current_notes = hier_gt[k]
        current_notes.forEach(function(n){
            // Determine horizontal coordinates.
            let oblongL = self.sk.map(
              n[0],
              // The endpoints in the music are the ontimes.
      ...
      let oblongL = self.sk.map(
        ...
        prm.screenLRepresents, totalBarCount*prm.beatsPerBar,
        // The endpoints in the graphics are determined by margin and width.
        prm.gridMargin*2, prm.canvasWidth - 1*prm.gridMargin
      )
      oblongL = Math.max(oblongL, prm.gridMargin)
      let oblongR = prm.gridMargin + self.sk.map(
        note_playing.offtime, prm.screenLRepresents, totalBarCount*prm.beatsPerBar,
        prm.gridMargin*2, prm.canvasWidth - 1*prm.gridMargin
      )
      // Prevent drawing outside of the grid.
      oblongR = Math.min(oblongR, prm.canvasWidth - 1*prm.gridMargin)
      // Determine vertical coordinate.
      const oblongV = self.sk.map(
        note_playing.MNN, prm.screenBRepresents, prm.screenTRepresents,
        prm.canvasHeight/2 - 2*prm.gridMargin, prm.gridMargin
      )
      // self.sk.rect(
      //   oblongL + prm.oblongNudge, oblongV + prm.canvasHeight/2 + 2*prm.oblongHeight,
      //   oblongR - oblongL - 2*prm.oblongNudge, prm.oblongHeight,
      //   1 // Little bit of rounding on the edges
      // )
      self.sk.stroke(100, 100, 130);
      self.sk.line(
        oblongL + prm.oblongNudge, prm.canvasHeight/2 + 3*prm.gridMargin, 
        oblongL + prm.oblongNudge, prm.canvasHeight
      );
    }
  ...
  /**
   * A class that extends the Buttons class and provides extra methods for touch checking.
   * @extends Button
   */

  class PlaypauseButtons extends Button {
    constructor(_sketch, _labelImg, _disabled, _clicked, _x, _y, _width, _height){
      super(_sketch, _labelImg, _disabled, _clicked, _x, _y, _width, _height);
    ...
    draw(newX){
      ...
      // this.sk.ellipse(this.x, this.y, this.w, this.h);
      this.sk.rect(this.x, this.y, this.w, this.h, this.h/5)
      this.sk.imageMode(this.sk.CENTER);
      this.sk.image(
        this.labelImg, this.x + this.w/2, this.y + this.h/2, 0.5*this.h,
        0.5*this.h
    ...
    touch_check(theVisual){
      if (
        ...
        // if (prm.printConsoleLogs) {
          console.log("GOT TO A TOUCH CHECK IN BUTTON!");
        // }
        this.onclick(theVisual);
        return true
      }
      return false
    }

    playpause_check(theVisual){
      const self = this;
      const checkResult = self.touch_check(theVisual);
        ...
        co.notes.forEach(function(n){
          Tone.Transport.schedule(function(time){
            pno.triggerAttackRelease(
              n.pitch, n.duration + 1, Tone.now(), n.tonejs.volume
              // "C4", 2, Tone.now(), 0.5
            )
            Tone.Draw.schedule(function(){
              self.draw_piano_roll_with_play_head(prm, n);
            }, Tone.now());
          }, n.ontime)
        })
  ...
  class StopButtons extends Button {
    constructor(_sketch, _disabled, _clicked, _x, _y, _width, _height){
      super(_sketch, _disabled, _clicked, _x, _y, _width, _height);
      // Any extra properties/actions here, which could have been
      // passed into the constructor also...
      // this.subject = subject
      this.sk = _sketch;
    ...
    draw(newX){
      ...
      if (this.disabled){
        ...
        this.sk.stroke(this.buttonStopColor);
        this.sk.fill(this.buttonStopColor);
      }
      // this.sk.ellipse(this.x, this.y, this.w, this.h);
      this.sk.rect(this.x, this.y, this.w, this.h, this.h/5)
      this.sk.fill(225);
      this.sk.rect(this.x + this.w/5*2, this.y + this.h/4, 0.5*this.h, 0.5*this.h)
    }
    ...
    touch_check(){
      if (
        ...
        // if (prm.printConsoleLogs) {
          console.log("GOT TO A TOUCH CHECK IN STOP BUTTON!");
        // }
        this.onclick();
        return true
      }
    ...
    playpause_check(){
      const self = this;
      const checkResult = self.touch_check();
    ...
    play_stop(co, prm){
      ...
      Tone.Transport.cancel()
      Tone.Transport.stop()
      self.draw_piano_roll_with_play_head(prm, co.notes[0])
    }

  }

  class TreeButton extends Button {
    constructor(_sketch, _text, _disabled, _clicked, _x, _y, _width, _height, _barIdx, _barCnt, _notes){
      super(_sketch, _text, _disabled, _clicked, _x, _y, _width, _height, _barIdx, _barCnt, _notes);
      this.sk = _sketch;
      // this.label = theLabel
      this.disabled = _disabled;
      this.clicked = _clicked;
      ...
      // passed into the constructor also...
      // this.subject = subject
      this.textFillDisabled = 40;
      this.textFillClicked = 40;
      this.textFill = 220;
      this.checkFlag = false;
      // For 1/4
      // this.barIdx = (_barIdx-1)*4 + 1;
      // this.barCnt = _barCnt * 4;
      // For 4/4
      this.barIdx = _barIdx;
      this.barCnt = _barCnt;
      this.notes = _notes
    ...
    draw(newX){
      ...
      if (this.disabled){
        ...
        this.sk.fill(this.buttonFillDisabled);
        this.sk.ellipse(this.x, this.y, this.w, this.h);
        this.sk.fill(this.textFillDisabled);
        this.sk.text(this.text, this.x + this.w/2, this.y + this.w/2);
      }
      else {
        if (this.clicked){
          this.sk.stroke(this.buttonStrokeClicked);
          this.sk.fill(this.buttonFillClicked);
          this.sk.ellipse(this.x, this.y, this.w, this.h);
          this.sk.fill(this.textFillClicked);
          this.sk.text(this.text, this.x + this.w/2, this.y + this.w/2);
        }
        else {
          this.sk.stroke(this.buttonStroke);
    ...
    touch_check(){
      if (
        ...
        // if (prm.printConsoleLogs) {
          console.log("GOT TO A TOUCH CHECK IN TREE BUTTON!");
        // }
        this.onclick();
        return true
      }
    ...
    playpause_check(){
      const self = this;
      const checkResult = self.touch_check();
      console.log("Touching tree button!")
    ...
    play_back(co, prm){
      ...
      if(Tone.Transport.state === "started"){
        ...
        console.log(">>>self.barIdx", self.barIdx);
        // console.log(">>>self.barCnt", self.barCnt);
        const currentOntime = this.notes[0][0]
        const endOntime = this.notes[this.notes.length - 1][0]
        console.log("Current ontime", self.barIdx)
        console.log("endOntime()", endOntime)
        // Let's try to play notes in an occurrence only.
        this.notes.forEach(function(n){
            Tone.Transport.schedule(function(time){
              pno.triggerAttackRelease(
                n[2], n[3]+1, Tone.now(), 0.5
              )
              Tone.Draw.schedule(function(){
                self.draw_piano_roll_with_play_head(prm, n);
              }, Tone.now());
...
//         // The code below will play notes in a region where the occurrence exist.
//         co.notes.forEach(function(n){
//           if(n.ontime >= currentOntime && n.ontime < endOntime){
//             Tone.Transport.schedule(function(time){
//               pno.triggerAttackRelease(
//                 n.pitch, n.duration + 1, Tone.now(), n.tonejs.volume
//               )
...
//         })
  ...
  */
  ...
  class TextInput {
    constructor(
      _sketch, _idDiv, _inputPlaceholder, _inputX, _inputY, _inputW, _btnText, _btnX, _btnY
    ){
      // this.sk = _sketch,
    ...
    mouse_pressed(){
      try {
        console.log("Mouse pressed the text input box: ", this.input.elt.value)
  ...
  var maiaGui = {
    Button: Button,
    Buttons: Buttons,
    PlaypauseButtons: PlaypauseButtons,
